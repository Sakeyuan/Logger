...
#define LOG_LEVEL(logger,level) \
    if(logger->getLevel() <= level) \
        LogEventWrap(LogEvent::ptr(new LogEvent(logger,level,__FILE__,__LINE__,0,Sake::GetThreadId(),Sake::GetFiberId(),time(0)))).getSS()
...
#define LOG_FMT_LEVEL(logger,level,fmt,...) \
    if(logger->getLevel() <= level) \
        ...
        Sake::GetThreadId(),Sake::GetFiberId(),time(0)))).getEvent()->format(fmt,__VA_ARGS__)
...
typedef Sake::Singleton<LoggerManager>LoggerMgr;
